...
function RequestDialog() {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={<AddButton label="Request adjustment" />} />
      <DialogContent>
        <DialogHeader>
        ...
        <DialogFooter>
          ...
          <LoadingButton loading={busy} loadingText="Submitting…" disabled={!(Number(amount) > 0) || reason.trim().length < 5} onClick={submit}>Submit for approval</LoadingButton>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
...
export function AdjustmentsView({ rows, canCreate, canApprove, currentUserId }: { rows: AdjustmentRow[]; canCreate: boolean; canApprove: boolean; currentUserId: string }) {
  ...
  const columns: ColumnDef<AdjustmentRow>[] = [
    ...
    actionsColumn<AdjustmentRow>((r) => <RowActions actions={[{ label: "View adjustment", icon: Eye, href: `/adjustments/${r.id}` }]} />),
  ]
